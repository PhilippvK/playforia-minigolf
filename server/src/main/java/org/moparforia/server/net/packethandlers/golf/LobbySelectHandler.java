...
public class LobbySelectHandler implements PacketHandler {
    ...
    public boolean handle(Server server, Packet packet, Matcher message) {
        if (message.group(1).equals("rnop")) {
            ...
            packet.getChannel().write("d lobbyselect\tnop\t" + Tools.tabularize(server.getLobby(LobbyType.SINGLE).totalPlayerCount(), 0, server.getLobby(LobbyType.MULTI).totalPlayerCount()));
